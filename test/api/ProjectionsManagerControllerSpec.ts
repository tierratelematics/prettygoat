...
describe("Given a ProjectionsController and a projection name", () => {
    let holder: Dictionary<IProjectionRunner<any>>,
        ...
        authorizationStrategy: TypeMoq.Mock<IAuthorizationStrategy>,
    ...
    beforeEach(
        () => {
            ...
            authorizationStrategy = TypeMoq.Mock.ofType(MockAuthorizationStrategy);
            response.setup(s => s.status(TypeMoq.It.isAny())).returns(a => response.object);
            subject = new ProjectionsManagerController(holder);
        }
    );

    context("when token is not authorized", () => {
        it("should not allow any command", () => {

        });
    });

    context("when token is authorized", () => {
